...
const readGoogleCredentialsFile = async (
  filePath: string
): Promise<Partial<GoogleCredentials>> => {
  ...
  if (!source) {
    throw new Error(
      'Invalid GOOGLE_OAUTH_CLIENT_JSON file format. Expected "installed" or "web" object.'
    )
  }
...
const resolveGoogleCredentials = async (): Promise<GoogleCredentials> => {
  ...
  const fileCredentials = credentialsPath
    ? await readGoogleCredentialsFile(credentialsPath)
    : {}
  ...
  if (!clientId) {
    throw new Error(
      'Missing GOOGLE_CLIENT_ID (or provide GOOGLE_OAUTH_CLIENT_JSON).'
    )
  }

  if (!clientSecret) {
    throw new Error(
      'Missing GOOGLE_CLIENT_SECRET (or provide GOOGLE_OAUTH_CLIENT_JSON).'
    )
  }
...
const startLocalCallbackServer = (): {
  ...
  try {
    ...
    return {
      waitForCode: Promise.reject(
        new Error('Static GOOGLE_REDIRECT_URI constant is not a valid URL.')
      ),
      close: async () => {},
      started: false
    }
  ...
  if (parsed.protocol !== 'http:' || !isLocalhost) {
    return {
      waitForCode: Promise.reject(
        new Error(
          'Automatic callback works only for local HTTP redirect URI (localhost/127.0.0.1).'
        )
      ),
      close: async () => {},
      started: false
    ...
    app.get(expectedPath, (req, res) => {
      ...
      if (error) {
        res
          ...
          .send(`OAuth error: ${error}. You can close this tab.`)
      ...
      if (!code) {
        res
          .status(400)
          .set('Connection', 'close')
          .send('No authorization code in callback. You can close this tab.')
      ...
      res
        .status(200)
        .set('Connection', 'close')
        .send('Authorization received. You can return to terminal.')
      resolve(code)
    })
...
if (isDirectExecution()) {
  main().catch((error: unknown) => {
    ...
    const normalized =
      `${responseData?.error || ''} ${responseData?.error_description || ''} ${message}`.toLowerCase()
